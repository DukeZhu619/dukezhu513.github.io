...
引理3表明，任一两点梯度估计器的二阶矩在 r→0r→0 时不会爆炸，因此相比于单点梯度估计器，在 rr 较小时能实现小得多的方差。基于这一优良性质，我们可以对采用两点估计器的 ZO-SGD 进行收敛性分析。
...
除了基于随机方向扰动的单点和两点梯度估计器，零阶优化还包括一类**基于标准坐标基向量**（standard basis vectors）的方法，称为**基于坐标的梯度估计器**（coordinate-wise gradient estimator）。这类方法不依赖于高斯或球面均匀分布的随机向量，而是直接沿坐标轴方向进行扰动，从而估计偏导数。
